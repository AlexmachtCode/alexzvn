...
| **2 · Video → NDI** | `onVideoRawDataReceived` (I420 nativ) → **mehrere NDI-Sender in EINEM Prozess** · Quelle „JM Connect – Zoom \<Name\>" (ohne Doppelpunkt, gemessen) erscheint ohne Switcher-Änderung. **Owner-Abnahme am 13./14.08.2026 gegen ein echtes Meeting weitgehend durch:** Bild fließt, Bild im NDI-Monitor angesehen (Lage + Farben richtig), Kamera aus/an mehrfach, **zwei Abos gleichzeitig mit je eigenem Zustand**, Meeting-Ende räumt die Abos ab. **Betriebsgröße: höchstens 5 gleichzeitige Zuschaltungen je Veranstaltung** (Owner). Offen: Weggang/Wiederbeitritt (`rebound`), ein Lauf mit 5 Gästen, Belastungstest zu Befund I6. [README Abschnitt 7](../packages/zoom-bridge/README.md). | 🟡 fast durch |
