...
| **3 · Ton je Person** | `onOneWayAudioRawDataReceived` (PCM16 je `node_id`) → NDI-Audio je Teilnehmer, in derselben Quelle wie das Bild. **Code fertig** (`packages/zoom-bridge/native/audio.cpp`, [README Abschnitt 7](../packages/zoom-bridge/README.md)). **Owner-Abnahme am echten Meeting offen, acht Punkte** ([Spec](superpowers/specs/2026-08-14-zoom-stage3-audio-ndi-design.md) §9): 1) Ton fließt und ist im NDI-Monitor **hörbar** (`waiting`→`live`); 2) Stummschalten und Aufheben, `live`↔`silent`, **kein Knacken**; 3) `audio:false` liefert Bild ohne Ton, gemeldet als `off`/`command`; 4) zwei Personen gleichzeitig: zwei Quellen, zwei getrennte Töne, keine Vermischung; 5) **Lippensynchronität** angesehen und angehört; 6) Weggang und Wiederbeitritt: der Ton kommt mit der umgehängten Quelle zurück; 7) Meeting-Ende meldet den Ton mit `meetingEnded`; 8) **gemessen mitgeschrieben**, welche Abtastrate und Kanalzahl Zoom tatsächlich liefert — und, aus der Schlussprüfung vom 18.08.2026 dazugekommen, **die tatsächliche Ankunftsrate der Ton-Pakete** (die Warteschlange in `native/audio.cpp` ist für fünf *gleichzeitig sprechende* Teilnehmer ausgelegt, während Zooms Rückruf für **jeden** Sprecher im Meeting feuert — bei vielen offenen Mikrofonen wird aus der halben Sekunde Vorrat ein Bruchteil) sowie **ob der Pegel plausibel ist** (`reference_level = 0` ist NDI-seitig zitiert, Zoom-seitig nur angenommen). | 🟡 Code fertig, Owner-Abnahme offen |
