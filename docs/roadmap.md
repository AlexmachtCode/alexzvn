...
| **3 · Ton je Person** | `onOneWayAudioRawDataReceived` (PCM16 je `node_id`) → NDI-Audio je Teilnehmer, in derselben Quelle wie das Bild. Code fertig, ein Ton-Abo braucht dabei `JoinVoip()` — ohne den Beitritt zum Tonkanal antwortet Zoom mit `SDKERR_NOT_JOIN_AUDIO` (32), und Video kennt diese Bedingung nicht (`packages/zoom-bridge/native/audio.cpp`, [README Abschnitt 7](../packages/zoom-bridge/README.md)). **Owner-Abnahme am echten Meeting: 5 von 8 Punkten durch (18.08.2026).** ✅ 1) Ton fließt und ist im NDI-Monitor **hörbar**; ✅ 2) `live`↔`silent` fünfmal über drei Abos, **kein Knacken** — die frühere Notiz „kein `silent`, Zoom sendet durchgehend“ war ein voreiliger Schluss aus zwei ereignislosen Läufen; ✅ 4) zwei Personen gleichzeitig, zwei Quellen, **im NDI-Monitor einzeln abgehört und sauber getrennt**; ✅ 6) Weggang und Wiederbeitritt: `black`/`off (participantLeft)` → `subscribed`/`waiting (reboundByName)` unter **neuer** Kennung, **Quellenname unverändert**, Ton zurück; ✅ 8) **gemessen mitgeschrieben:** 32 kHz Mono, 320 Abtastwerte je Paket, rund 100 Pakete je Sekunde und Sprecher (= genau 10 ms je Paket). Dazu **erstmals die Summenrate mit ZWEI gleichzeitigen Sprechern: 101 + 102 Pakete/s, kein `AUDIO_QUEUE_OVERFLOW`** — über fünf sagt das weiterhin nichts. ❌ **5) Lippensynchronität: der Ton läuft dem Bild hinterher** (Owner, 18.08.2026). Passt zu einer baulichen Unsymmetrie: **Bild** geht direkt im SDK-Rückruf raus (`Delegate::onRawDataFrameReceived` → `sendI420()`), **Ton** erst beim Tick über die Warteschlange — und beide tragen `NDIlib_send_timecode_synthesize`, also „stemple mit jetzt“. `ndi_sender.cpp` hat den Fall vorweggenommen: eigene Zeitstempel aus Zooms `GetTimeStamp()` seien erst richtig, *wenn gemessen ist*, dass die synthetische Taktung auseinanderläuft. **Noch NICHT gemessen ist, ob der Versatz gleich bleibt oder wächst** — das trennt Rohrleitungs-Verzögerung von Abtastwert-Buchhaltung und entscheidet die Abhilfe. ✅ 3) `audio:false` liefert Bild ohne Ton, gemeldet als `off`/`command` (**nicht** `audioUnavailable` — der Unterschied trennt „abgeschaltet“ von „bekam keinen“); ✅ 7) Meeting-Ende meldet den Ton **eigens** mit `meetingEnded`. ⚑ **Dabei ein Absturz gefunden und behoben:** das Meeting-Ende beendete den Prozess mit `0xC0000005` — ein seit **Stage 2** vorhandener Fehler, der unsichtbar war, weil der Rückgabewert des Kindprozesses nie angezeigt wurde. Drei Anläufe, zwei davon durch Messung widerlegt (Befund I6, dann Re-Entranz), bis die Ursache stand: nach dem Meeting-Ende ist der Renderer bereits abgeräumt und darf nicht mehr angefasst werden. Offen bleibt nur noch **ob der Pegel plausibel ist** (`reference_level = 0` ist NDI-seitig zitiert, Zoom-seitig angenommen). ⛑ **Betriebshinweis, gemessen:** die Quelle nicht über Lautsprecher abhören, wenn das abonnierte Mikrofon im selben Raum steht — der Ton läuft sonst akustisch im Kreis und klingt doppelt. Nicht die Brücke: sie sendet nachweislich 101 % der angegebenen Rate, und mit Kopfhörern ist die Dopplung weg. | 🟡 Abnahme 7/8, nur Punkt 5 offen |
