...
| **3 · Ton je Person** | `onOneWayAudioRawDataReceived` (PCM16 je `node_id`) → NDI-Audio je Teilnehmer, in derselben Quelle wie das Bild. Code fertig, ein Ton-Abo braucht dabei `JoinVoip()` — ohne den Beitritt zum Tonkanal antwortet Zoom mit `SDKERR_NOT_JOIN_AUDIO` (32), und Video kennt diese Bedingung nicht (`packages/zoom-bridge/native/audio.cpp`, [README Abschnitt 7](../packages/zoom-bridge/README.md)). **Owner-Abnahme am echten Meeting: 2 von 8 Punkten durch (18.08.2026).** ✅ 1) Ton fließt und ist im NDI-Monitor **hörbar** (`waiting`→`live`); ✅ 8) **gemessen mitgeschrieben:** Zoom liefert **32 kHz Mono, 320 Abtastwerte je Paket, rund 100 Pakete je Sekunde und Sprecher** (= genau 10 ms je Paket) — die Brücke schreibt es beim ersten Abo selbst auf stderr mit. Offen: 2) `live`↔`silent`, **kein Knacken** — der Übergang nach `silent` **ist** im dritten Lauf gekommen (18.08., `audio 16778240: silent (gap)`); die frühere Notiz „kein `silent`, Zoom sendet durchgehend" war ein voreiliger Schluss aus zwei ereignislosen Läufen. Offen bleibt, **was** ihn auslöst (Stummschaltung oder Sprechpause), der Rückweg nach `live` und ob es knackt; 3) `audio:false` liefert Bild ohne Ton (`off`/`command`); 4) zwei Personen gleichzeitig, zwei getrennte Töne; 5) **Lippensynchronität**; 6) Weggang und Wiederbeitritt bringen den Ton mit der umgehängten Quelle zurück; 7) Meeting-Ende meldet den Ton mit `meetingEnded`; sowie die **Summenrate bei mehreren gleichzeitig Sprechenden** (gemessen wurde ein Sprecher; dass fünf linear 500/s ergeben, ist Arithmetik) und **ob der Pegel plausibel ist** (`reference_level = 0` ist NDI-seitig zitiert, Zoom-seitig angenommen). ⛑ **Betriebshinweis, gemessen:** die Quelle nicht über Lautsprecher abhören, wenn das abonnierte Mikrofon im selben Raum steht — der Ton läuft sonst akustisch im Kreis und klingt doppelt. Nicht die Brücke: sie sendet nachweislich 101 % der angegebenen Rate, und mit Kopfhörern ist die Dopplung weg. | 🟡 Code fertig, Abnahme 2/8 |
